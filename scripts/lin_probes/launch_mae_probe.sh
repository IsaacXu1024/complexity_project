#!/bin/bash
#SBATCH --time=00-01:30:00          # max walltime, hh:mm:ss
...
do
	echo "Filter = ${filter}"
	srun python ../../main.py \
		--ckpt_path "../../checkpoints/mae_pre-train/odz5zdrg/mae_pre-train-odz5zdrg-ep=299.ckpt" \
		--train_cfg "lp_mae_vit_s.cfg" \
		--aug_stack_cfg "lp.cfg" \
		--filter_cfg "$filter" \
		--nodes "$SLURM_JOB_NUM_NODES" \
		--gpus "$GPUS_PER_NODE" \
		--name "mae_vit_lp_${filter}" \
		--filter_train true \
		--filter_test true
done