#!/bin/bash
#SBATCH --time=00-00:50:00          # max walltime, hh:mm:ss
...
do
	echo "Filter = ${filter}"
	srun python ../../main.py \
		--ckpt_path "../../checkpoints/mocov3_pre-train/2bywsxji/mocov3_pre-train-2bywsxji-ep=299.ckpt" \
		--train_cfg "lp_mocov3_vit_s.cfg" \
		--aug_stack_cfg "lp.cfg" \
		--filter_cfg "$filter" \
		--nodes "$SLURM_JOB_NUM_NODES" \
		--gpus "$GPUS_PER_NODE" \
		--name "mocov3_vit_lp_${filter}" \
		--filter_train true \
		--filter_test true
done